...
import { LoginSchema, type LoginFormValues, type LoginApiResponse } from "@/lib/types/auth";
...
export default function LoginForm({ csrfToken }: LoginFormProps) {
  "use no memo"; // react-hook-form's watch() is incompatible with the React Compiler
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    try {
      ...
      const { role, user } = (await res.json()) as LoginApiResponse;
      setUser(user);
      queryClient.clear();
      router.replace(ROLE_ROUTE_PREFIX[role]);
    } catch {
