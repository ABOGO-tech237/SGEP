...
@shared_task(bind=True, max_retries=3)
def generate_attendance_excel_task(self, job_id: str, params: dict | None = None) -> None:
	try:
		ReportJobRepository.update(job_id, {"status": "processing"})
		params = params or {}
		file_path = excel_export.export_attendance(
			params.get("class_id", ""),
...
@shared_task(bind=True, max_retries=3)
def generate_results_excel_task(self, job_id: str, params: dict | None = None) -> None:
	try:
		ReportJobRepository.update(job_id, {"status": "processing"})
		params = params or {}
		file_path = excel_export.export_results(
			params.get("class_id", ""),
